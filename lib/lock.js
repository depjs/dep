...
// optionalDependencies entries override same-named dependencies entries (npm
// semantics), so they merge last and their names stay optional.
...
const optionalOf = (json) => Object.keys(json.optionalDependencies || {})
...
const lock = (argv) => {
  ...
  // A name required by anyone isn't optional — but a manifest that also lists
  // the name in its own optionalDependencies keeps it optional.
  Object.keys(deps).forEach((name) => {
    const requiredSomewhere = [pkgJSON, ...workspaces.map((ws) => ws.pkg)].some((j) =>
      !(j.optionalDependencies && j.optionalDependencies[name]) &&
