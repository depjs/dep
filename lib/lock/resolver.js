...
const resolver = (dep, deps, base, resolve, reject) => {
  ...
  fetcher(dep, deps[dep]).then((pkg) => {
    ...
    // Keep the requested ranges for the lockfile; `dependencies` is reused by
    // tree.setter to hold the resolved children, so move the ranges aside.
    item.requires = pkg.dependencies
