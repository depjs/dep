...
const reachable = (packages, roots, withOptional) => {
  ...
  while (stack.length) {
    ...
    if (!entry) continue
    // A name in optionalDependencies is an optional edge even when it also
    // appears in dependencies (npm merges optionals into dependencies when
    // publishing), so the strict pass must not follow it.
    const edges = Object.assign({}, entry.dependencies)
    if (withOptional) Object.assign(edges, entry.optionalDependencies)
    else Object.keys(entry.optionalDependencies || {}).forEach((name) => { delete edges[name] })
