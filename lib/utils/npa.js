...
const parseGit = (spec) => {
  ...
  let auth = null
  let m
  // `git` is the transport user every host expects for ssh, not a credential:
  // carrying it into the https templates builds a url fetch() refuses to
  // construct. Drop it in both url forms, keep any other user as auth.
  const authOf = (user) => user && user !== 'git' ? user : null
...
export default (arg, where) => {
  ...
  // Registry result for a (target) name + spec.
  const registry = (resolvedName, resolvedSpec, alias) => {
    let type
    if (resolvedSpec === '') type = 'range'
    else if (semver.valid(resolvedSpec)) type = 'version'
    else if (semver.validRange(resolvedSpec)) type = 'range'
    else type = 'tag'
    return {
      type,
      name,
      alias,
      escapedName: resolvedName.replace('/', '%2f'),
      fetchSpec: resolvedSpec === '' ? '*' : resolvedSpec,
      hosted: null
    }
  }

  // Aliases: `npm:other@range` installs `other` under this dependency's name.
  if (spec && spec.startsWith('npm:')) {
    const [target, targetSpec] = splitNameSpec(spec.slice(4))
    return registry(target, targetSpec, target)
  }
