...
import matchesPlatform, { describePlatform } from './platform.js'
...
export const CONCURRENCY = Math.max(1,
...
// (non-optional) peerDependencies, and optionalDependencies. A name listed in
// optionalDependencies is optional even when it also appears in dependencies:
// npm merges optionals into dependencies when publishing (e.g.
// jest-haste-map's darwin-only fsevents sits in both), and npm's rule is that
...
const edgesOf = (meta) => {
  ...
  for (const name of Object.keys(deps)) {
    if (bundled.has(name) || name in optionals) continue
  ...
  for (const name of Object.keys(peers)) {
    if (name in deps || name in optionals || bundled.has(name)) continue
...
export default (deps, fetcher, opts = {}) => {
  const {
    ...
    engineStrict = false,
    overrides = new Map(),
    // Called with each package's metadata as soon as it resolves — the
    // install resolver uses it to start tarball downloads during resolution.
    onMeta = null
  } = opts
  ...
  const build = () => {
    ...
    while (queue.length) {
      ...
      if (skipPlatform && !matchesPlatform(meta)) {
        ...
        if (!optional) {
          platformErrors.push(`${id} (${describePlatform(meta)})`)
        }
        continue
      }
      ...
      // Whether this package is reachable only through optional edges — the
      // installer uses it to decide if a failed native build is skippable.
      item.optional = optional && !required.has(cacheKey(name, range, cscope))
