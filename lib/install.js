...
const install = (argv) => {
  ...
  if (cluster.isWorker) {
    process.on('message', (msg) => {
      if (!msg.mejiro) return
      const tasks = installer(msg.shared)
      ...
      Promise.all(tasks.slice(begin, end)).then((values) => {
        process.send({index: index, values: values})
      }).catch(() => {process.send({index: index})})
    })
    return
  }
  ...
  Promise.all(list).then(() => {
    ...
    fs.removeSync(nm)
    Mejiro.all(tasks, global.dependenciesTree).then(() => {
