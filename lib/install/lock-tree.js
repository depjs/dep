...
import matchesPlatform, { describePlatform } from '../utils/platform.js'
...
// not v3, or stale relative to the requested top-level deps. Throws when a
// required (non-optional) entry cannot run on the current platform.

const nodeFromEntry = (entry, cwd, name) => {
  ...
  if (/^git\+/.test(resolved) || /\.git(#|$)/.test(resolved)) {
    ...
    // npm omits `resolved` (and `integrity`) for packages it installed from
    // its local cache; rebuild the conventional registry tarball URL from the
    // package name and version, the same way npm re-derives it.
    const pkgName = entry.name || name
    node.type = 'registry'
    node.tarball = resolved ||
      `${npmrc.registry}${pkgName}/-/${pkgName.split('/').pop()}-${entry.version}.tgz`
...
export default (cwd, topLevel, overrides) => {
  ...
  // The lockfile legitimately records every platform's optional variants
  // (lightningcss-darwin-arm64 next to -linux-x64-gnu, …); npm installs only
  // the entries that match the current os/cpu/libc and are reachable from the
  // root through non-skipped packages. Walk each entry's recorded dependency
  // edges from the root, resolving names up the node_modules chain the way
  // require() does: an optional entry for another platform is silently
  // dropped along with anything reachable only through it (e.g. a skipped
  // wasm binding's own runtime helper), while a *required* entry that cannot
  // run here fails the install, like npm's EBADPLATFORM.
  const resolveFrom = (base, name) => {
    while (true) {
  ...
  // Every dependency edge a lock entry records. devDependencies exist only on
  // the root and workspace-source entries, whose dev deps do get installed.
  const edgesOf = (entry) => Object.keys(Object.assign({},
    entry.dependencies, entry.devDependencies,
    entry.optionalDependencies, entry.peerDependencies))

  const included = new Set()
  const skipped = new Set()
  const platformErrors = []
  // Seed with the root entry's edges plus the requested top-level names, so a
  // lock whose root entry is sparse still covers everything package.json asks.
  const stack = ['']
  for (const name of topLevel) {
    const location = resolveFrom('', name)
    if (location) stack.push(location)
  }
  while (stack.length) {
    const location = stack.pop()
    if (included.has(location) || skipped.has(location)) continue
    const entry = lock.packages[location]
    if (!entry) continue
    if (location !== '' && !matchesPlatform(entry)) {
      ...
      if (!entry.optional && !entry.devOptional) {
        ...
        platformErrors.push(`${name}@${entry.version} (${describePlatform(entry)})`)
      }
      continue
    }
    included.add(location)
    // A workspace link's edges live on its source entry, resolved from there.
    let base = location
    let source = entry
    if (entry.link) {
      base = entry.resolved
      source = lock.packages[base]
      if (!source) continue
      included.add(base)
    }
    for (const name of edgesOf(source)) {
      const child = resolveFrom(base, name)
  ...
  for (const location of Object.keys(lock.packages)) {
    ...
    if (!included.has(location)) continue // other platform's variant, or unreachable
    ...
    segments.forEach((name, i) => {
      if (i === segments.length - 1) {
        ...
        ref[name] = nodeFromEntry(entry, cwd, name)
