...
const resolver = (dep, deps, base, resolve, reject) => {
  ...
  for (let i = 0; base.length > i; i += 2) {
    let target = tree.getter(global.dependenciesTree, base.slice(0, i))
  ...
  fetcher(dep, deps[dep]).then((pkg) => {
    if (!global.dependenciesTree[dep]) {
      ...
      global.dependenciesTree[dep].tarball = pkg.tarball
      global.dependenciesTree[dep].shasum = pkg.shasum
    } else {
      tree.setter(global.dependenciesTree, dep, {
        version: pkg.version,
        tarball: pkg.tarball,
        shuasum: pkg.shasum
      }, base)
